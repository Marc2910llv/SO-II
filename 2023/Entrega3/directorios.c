                   ...
                   unsigned int *p_inodo, unsigned int *p_entrada, char reservar, unsigned char permisos)
{
    ...
    num_entrada_inodo = 0;
    int tamany = sizeof(struct entrada); // nº de entrada inicial

    if (cant_entradas_inodo > 0)
    {
        if (mi_read_f(*p_inodo_dir, &entrada, num_entrada_inodo * tamany, tamany) < 0)
        {
            return ERROR_PERMISO_LECTURA;
        }
...
/// @param camino
/// @param p_stat
/// @return p_inodo o error o FALLO
...
int mi_read(const char *camino, void *buf, unsigned int offset, unsigned int nbytes)
{
    ...
    for (int i = 0; i < (auxCACHE - 1); i++)
    {
        if (strcmp(camino, UltimaEntradaEscritura[i].camino) == 0)
        {
            ...
            diferenteInodo = 0;
#if DEBUG2
            fprintf(stderr, RED "[mi_read() → Usamos CACHÉ[%i]]\n" RESET, i);
#endif
            break;
        }
    }
    ...
    if (diferenteInodo)
    {
        ...
        {
            perror("Error mi_read buscar_entrada");
            return FALLO;
        }
        ...
        {
#if DEBUG2
            fprintf(stderr, RED "[mi_read() --> Hemos añadido una entrada en CACHÉ[%i]]\n" RESET, CACHE - auxCACHE);
#endif
            strcpy(UltimaEntradaEscritura[CACHE - auxCACHE].camino, camino);
            UltimaEntradaEscritura[CACHE - auxCACHE].p_inodo = p_inodo;
            --auxCACHE;
        ...
        else
        {
            ...
            {
#if DEBUG2
                fprintf(stderr, RED "[mi_read() --> Pasamos contenido de CACHÉ[%i] a CACHÉ[%i]]\n" RESET, i + 1, i);
#endif
                strcpy(UltimaEntradaEscritura[i].camino, UltimaEntradaEscritura[i + 1].camino);
                UltimaEntradaEscritura[i].p_inodo = UltimaEntradaEscritura[i + 1].p_inodo;
            }
    ...
    {
        bytes_leidos = 0;
        perror("Error mi_read mi_read_f");
    }
...
int mi_link(const char *camino1, const char *camino2)
{
    ...
    if (inodo.tipo != 'f')
    {
        fprintf(stderr, RED "Error: se trata de un fichero\n" RESET);
        mi_signalSem();
        return FALLO;
    }
    ...
    if (mi_read_f(p_inodo_dir2, &entrada2, sizeof(struct entrada) * (p_entrada2), sizeof(struct entrada)) < EXITO)
    {
    ...
    if (mi_write_f(p_inodo_dir2, &entrada2, sizeof(struct entrada) * (p_entrada2), sizeof(struct entrada)) < EXITO)
    {
    ...
    if (liberar_inodo(p_inodo2) < EXITO)
    {
