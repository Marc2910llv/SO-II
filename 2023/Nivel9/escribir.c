...
int main(int argc, char *argv[])
{
    ...
    {
        fprintf(stderr, RED "Error main bmount" RESET);
        return FALLO;
    }
    ...
    if (ninodo == FALLO)
    {
        fprintf(stderr, RED "Error main reservar_inodo" RESET);
        return FALLO;
    }
    if (ninodo == -2)
    {
        fprintf(stderr, RED "Error main reservar_inodo (no quedan bloques libres)" RESET);
        return FALLO;
    }
    ...
    for (int i = 0; i < (sizeof(offset) / sizeof(int)); i++)
    {
        ...
        {
            fprintf(stderr, RED "Error main mi_write_f" RESET);
            return FALLO;
        }
